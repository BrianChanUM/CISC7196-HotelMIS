...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'delete') {
        if (!checkPermission('admin_vehicles', 'delete')) {
...
<body>
    ...
    <div id="tf-about">
        <div class="container">
            <div class="row">
                ...
                <div class="tab">
                    <input type="text" id="searchInput" onkeyup="searchTable()" placeholder="Search for vehicle types..">
                ...
                <div id="All" class="tabcontent" style="display:block">
                    <table id="vehicleTableAll" class="table table-bordered table-striped table-hover" style="font-size: 14px;">
                        <thead>
                            <tr>
                                <th>Vehicle Type</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>";
                                    if (checkPermission('admin_vehicles', 'edit')) {
                                        echo "<button class='action-btn edit-btn' onclick='openEditModal(\"" . htmlspecialchars($row['VehicleType']) . "\")'>Edit</button>";
                                    }
                                    if (checkPermission('admin_vehicles', 'delete')) {
                                        echo "<button class='action-btn delete-btn' onclick='deleteVehicleType(\"" . htmlspecialchars($row['VehicleType']) . "\")'>Delete</button>";
                                    }
                                    echo "</td>";
                                    echo "</tr>";
                                }
                            } else {
                                echo "<tr><td colspan='4'>No vehicle types found</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
                ...
                <div id="Active" class="tabcontent" style="display:none">
                    <table id="vehicleTableActive" class="table table-bordered table-striped table-hover" style="font-size: 14px;">
                        <thead>
                            <tr>
                                <th>Vehicle Type</th>
                                <th>Daily Quantity</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                echo "<tr><td colspan='4'>No active vehicle types found</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
                ...
                <div id="Inactive" class="tabcontent" style="display:none">
                    <table id="vehicleTableInactive" class="table table-bordered table-striped table-hover" style="font-size: 14px;">
                        <thead>
                            <tr>
                                <th>Vehicle Type</th>
                                <th>Daily Quantity</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                echo "<tr><td colspan='4'>No inactive vehicle types found</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
    ...
    <div id="editModal" class="modal" style="display:none; position:fixed; z-index:1; left:0; top:0; width:100%; height:100%; background-color:rgba(0,0,0,0.5);">
        <div class="modal-content" style="background-color:#fefefe; margin:15% auto; padding:20px; border:1px solid #888; width:50%; max-width:500px;">
            ...
            <form id="editForm">
                <input type="hidden" id="oldVehicleType" name="oldVehicleType">
                
                <label for="vehicleType">Vehicle Type:</label>
                <input type="text" id="vehicleType" name="vehicleType" required style="width:100%; padding:8px; margin:5px 0;"><br>
    ...
    function searchTable() {
        ...
        var tableId = "vehicleTableAll";
        if (activeTab) {
            var tabName = activeTab.textContent.trim();
            if (tabName === "Active") {
                tableId = "vehicleTableActive";
            } else if (tabName === "Inactive") {
                tableId = "vehicleTableInactive";
            }
        }
        
        var table = document.getElementById(tableId);
        if (!table) {
            table = document.getElementById("vehicleTableAll");
        }
    ...
    function openEditModal(vehicleType) {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "managevehicletype.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function() {
            if (this.readyState === 4 && this.status === 200) {
                var response = JSON.parse(this.responseText);
                if (response.success) {
                    document.getElementById("oldVehicleType").value = vehicleType;
                    document.getElementById("vehicleType").value = response.data.VehicleType;
        ...
        xhr.send("action=get&vehicleType=" + encodeURIComponent(vehicleType));
    }
    ...
    document.getElementById("editForm").addEventListener("submit", function(e) {
        ...
        formData.append("oldVehicleType", document.getElementById("oldVehicleType").value);
    ...
    function deleteVehicleType(vehicleType) {
        if (confirm("Are you sure you want to delete this vehicle type?")) {
            ...
            xhr.send("action=delete&vehicleType=" + encodeURIComponent(vehicleType));
        }
    }
